...
class CRES_Dataset(torch.utils.data.Dataset):
    ...
    def apply_max_pooling(self, imgs, targets): 
        ...
        targets_mp, indices = maxpool(targets.float()) # Not sure if to use the indices or not here...
    ...
    def load_spec_dir(self, dir_path):
        ...
        for file in files:
            ...
            img = spec_to_numpy(file, freq_bins = self.freq_bins)
    ...
    def spec_to_numpy(self, spec_path, slices=-1, packets_per_slice=1, start_packet=None):
        """
        TODO: Document.
...
class CRES_DM(pl.LightningDataModule):
    ...
    def __init__(
        ...
        max_pool = 8,
        transform=None, 
        train_val_test_splits=(.6, .3, .1),
        batch_size=1,
        shuffle_dataset=True,
        seed = 42,
        class_map={
            0: {"name": "background","target_color": (0,0,0),},
    ...
    def setup(self, stage = None):
        ...
        self.cres_dataset  = CRES_Dataset(self.root_dir , freq_bins = self.freq_bins, max_pool =self.max_pool, transform=self.transform)

        # Creating data indices for training and validation splits:
        dataset_size = len(self.cres_dataset)
        indices = list(range(dataset_size))
        splits = self.train_val_test_splits
        split_idxs = [int(np.floor(splits[0] * dataset_size)),  int(np.floor((splits[0]  + splits[1]) * dataset_size))]

        if self.shuffle_dataset :
            rng = np.random.default_rng(self.seed)
            rng.shuffle(indices)

        train_indices, val_indices, test_indices = indices[:split_idxs[0]], indices[split_idxs[0]:split_idxs[1]], indices[split_idxs[1]:]
